...
class OxfordPets(DatasetBase):
    ...
    #so what the bottom method is doing is
        #1) first the json file is read
        #2) we take the test, val and train splits
        #3) for each one, we make a list of Datum objects which need the following
            #Image path (from json + the image directory)
            #Label
            #Classname
    @staticmethod
