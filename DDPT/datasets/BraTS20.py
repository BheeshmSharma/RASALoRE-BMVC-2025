...
class BraTS20(DatasetBase):

    dataset_dir_name = "BraTS20"

    def __init__(self, cfg):
        
        root = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))
        self.dataset_dir = os.path.dirname(root)
        self.image_dir = root
        
        if(cfg.DATASET.TRAIN_PERCENT!=0):  #THIS IS RUN DURING TRAINING
          self.split_path = os.path.join(self.dataset_dir, f"split_brats20_{os.path.basename(root)}.json")
          self.split_fewshot_dir = os.path.join(self.dataset_dir, f"split_fewshot_train_{os.path.basename(root)}")
        else:                              #THIS IS RUN DURING TESTING
          self.split_path = os.path.join(self.dataset_dir, f"split_brats20_full_{os.path.basename(root)}.json")
          self.split_fewshot_dir = os.path.join(self.dataset_dir, f"split_fewshot_test_{os.path.basename(root)}")
